...
class ArucoDetectorTestFixture : public ::testing::Test {
   protected:
    ...
    void SetUp() override {
        ...
        markerSize = 0.167;
        double intrinsic_params[] = {1050.0, 1050.0, 960.0,
                                     540.0};  // fx, fy, cx, cy
        double distortion_params[] = {
            0.000335365051980971, 0.000583836572965934, 0.0, 0.0,
            0.000318839213604595};  // k1, k2, p1, p2, k3
        cv::Mat camera_matrix = (cv::Mat_<double>(3, 3) << intrinsic_params[0],
                                 0, intrinsic_params[2], 0, intrinsic_params[1],
                                 intrinsic_params[3], 0, 0, 1);

        cv::Mat distortion_coefficients =
            (cv::Mat_<double>(1, 5) << distortion_params[0],
             distortion_params[1], distortion_params[2], distortion_params[3],
             distortion_params[4]);
        ...
        detector = std::make_unique<vortex::aruco_detector::ArucoDetector>(
            dictionary, markerSize, cameraMatrix, distortionCoeffs,
            detectorParams);
        float xDist = 0.462;
        float yDist = 0.862;
        std::vector<int> ids = {28, 7, 96, 19};
        board = detector->createRectangularBoard(markerSize, xDist, yDist,
                                                 dictionary, ids);
    }
};
...
TEST_F(ArucoDetectorTestFixture, CreateRectangularBoardTest) {
    ...
    cv::Ptr<cv::aruco::Board> board = detector->createRectangularBoard(
        markerSize, xDist, yDist, dictionary, ids);

    // Assert that the board is not null
    ASSERT_TRUE(board != nullptr);

    // Create an image file from the board
    std::string boardImageFile =
        "/home/jorgen/ros2_ws/src/vortex-sensor-fusion/detectors/"
        "aruco_detector/test/board_image.jpg";
...
TEST_F(ArucoDetectorTestFixture, DetectMarkersTest) {
    ...
    auto [marker_corners, rejectedCandidates, marker_ids] =
        detector->detectArucoMarkers(imageCopy);
    ...
    std::string outputImageFile =
        "/home/jorgen/ros2_ws/src/vortex-sensor-fusion/detectors/"
        "aruco_detector/test/output_image.jpg";
...
TEST_F(ArucoDetectorTestFixture, EstimateBoardPoseTest) {
    ...
    auto [boardId, rvec, tvec] =
        detector->estimateBoardPose(marker_corners, marker_ids, board);

    float length =
        cv::norm(board->objPoints[0][0] -
                 board->objPoints[0][1]);  // Visual length of the drawn axis
    cv::aruco::drawAxis(imageCopy, cameraMatrix, distortionCoeffs, rvec, tvec,
                        length);
    ...
    std::string outputImageFile =
        "/home/jorgen/ros2_ws/src/vortex-sensor-fusion/detectors/"
        "aruco_detector/test/drawn_board_image.jpg";
    cv::imwrite(outputImageFile, imageCopy);
}
