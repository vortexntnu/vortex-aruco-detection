...
ArucoDetectorNode::ArucoDetectorNode(const rclcpp::NodeOptions& options)
    ...
    this->declare_parameter<std::vector<double>>(
        "camera.intrinsic", {1050.0, 1050.0, 960.0, 540.0});
    this->declare_parameter<std::vector<double>>(
        "camera.distortion", {0.000335365051980971, 0.000583836572965934, 0.0,
                              0.0, 0.000318839213604595});
...
void ArucoDetectorNode::setCameraParams() {
    std::vector<double> intrinsic_params =
        this->get_parameter("camera.intrinsic").as_double_array();
    std::vector<double> distortion_params =
        this->get_parameter("camera.distortion").as_double_array();
    ...
    camera_matrix_ =
        (cv::Mat_<double>(3, 3) << intrinsic_params[0], 0, intrinsic_params[2],
         0, intrinsic_params[1], intrinsic_params[3], 0, 0, 1);

    distortion_coefficients_ =
        (cv::Mat_<double>(1, 5) << distortion_params[0], distortion_params[1],
...
void ArucoDetectorNode::checkAndSubscribeToCameraTopics() {
    ...
    std::string image_topic =
        this->get_parameter("subs.image_topic").as_string();
    std::string camera_info_topic =
        this->get_parameter("subs.camera_info_topic").as_string();
    ...
    if (camera_info_topic_ != camera_info_topic) {
        ...
        RCLCPP_INFO(this->get_logger(), "Subscribed to camera info topic: %s",
...
void ArucoDetectorNode::cameraInfoCallback(
    ...
    if (msg->k.empty() || msg->d.empty()) {
        RCLCPP_ERROR(this->get_logger(),
                     "Received camera info with empty calibration data.");
        return;
    }

    // Map camera matrix
    camera_matrix_ = (cv::Mat_<double>(3, 3) << msg->k[0], 0, msg->k[2], 0,
                      msg->k[4], msg->k[5], 0, 0, 1);

    if (msg->distortion_model == "plumb_bob") {
        // Map distortion coefficients
        distortion_coefficients_ =
            (cv::Mat_<double>(1, 5) << msg->d[0], msg->d[1], 0, 0, msg->d[4]);
    } else {
        RCLCPP_WARN(this->get_logger(), "Unsupported distortion model: %s",
    ...
    std::string formatted_string2 =
        oss2.str();  // Now correctly using oss2 for the camera matrix

    RCLCPP_INFO(this->get_logger(), "Distortion coefficients: %s",
                formatted_string1.c_str());
    RCLCPP_INFO(this->get_logger(), "Camera matrix: %s",
                formatted_string2.c_str());

    RCLCPP_INFO(this->get_logger(),
                "Camera info received, subscription will be terminated.");
    camera_info_sub_
        .reset();  // Terminate subscription after receiving camera info
    initializeDetector();
}
...
void ArucoDetectorNode::initializeDetector() {
    ...
    if (dictionary_map.count(dictionary_type)) {
        ...
        RCLCPP_WARN(this->get_logger(),
                    "Invalid dictionary type received: %s. Using default.",
                    dictionary_type.c_str());
        dictionary_ =
...
void ArucoDetectorNode::onParameterEvent(
    ...
    for (const auto& changed_parameter : event.changed_parameters) {
        if (changed_parameter.name.find("aruco.") == 0)
            aruco_changed = true;
        else if (changed_parameter.name.find("detect_board") == 0)
            detect_board_changed = true;
        else if (changed_parameter.name.find("board.") == 0)
            board_changed = true;
        else if (changed_parameter.name.find("camera.") == 0)
            camera_changed = true;
        else if (changed_parameter.name.find("camera_frame") == 0)
            setFrame();
        else if (changed_parameter.name.find("subs.") == 0)
            checkAndSubscribeToCameraTopics();
    }
    // Directly invoke initializer functions if their related parameters have
    // changed
    if (aruco_changed) {
        initializeDetector();
    }
...
void aruco_detector::ArucoDetectorNode::imageCallback(
    ...
    if (detect_board_) {
        ...
        if (valid > 0) {
            ...
            geometry_msgs::msg::PoseStamped pose_msg =
                cv_pose_to_ros_pose_stamped(board_tvec, board_quat, frame_,
                                            msg->header.stamp);
    ...
    for (size_t i = 0; i < marker_ids.size(); i++) {
        ...
        auto pose_msg =
            cv_pose_to_ros_pose_stamped(tvec, quat, frame_, msg->header.stamp);
